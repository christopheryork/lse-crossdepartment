...
queue().defer(d3.csv, "../data-6.2.csv").defer(d3.csv, "../data-6.4.csv").await(function (err, research, teaching) {
  if (err) {
    throw err;
  }
  ...
  var scale = d3.scale.ordinal().domain(d3.range(n)).rangeRoundBands([0, width - padding], 0.1);
  ...
  var g = matrix.selectAll("g").data(points).enter().append("g").attr("transform", function (d) {
    return "translate(" + scale(d.i) + "," + (padding + scale(d.j)) + ")";
  }).attr("opacity", function (d) {
    return d.i > d.j ? "1.0" : "0.5";
  });
  ...
  g.append("rect").attr("rx", 1).attr("ry", 1);
  ...
  svg.append("g").attr("class", "x_labels").attr("transform", "translate(0," + padding + ")").selectAll("text").data(dept_names).enter().append("g").attr("transform", function (d, i) {
    return "translate(" + (scale(i) + 5) + ",-5)rotate(-90)";
  ...
  function update() {
    ...
    if (details) {
      ...
      svg.selectAll(".matrix rect").attr("stroke", "grey")
      /*          .attr("x", 0)
                .attr("y", 0) */
      .attr("width", scale.rangeBand()).attr("height", scale.rangeBand()).attr("fill", "transparent").attr("opacity", "0.2");
    } else {
      svg.selectAll(".matrix circle").transition().attr("visibility", "hidden");
      svg.selectAll(".matrix rect").attr("stroke", "none")
